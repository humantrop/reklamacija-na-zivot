...
export async function sendWelcomeEmail(to: string) {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #0a0a12; color: #e2e8f0; border-radius: 16px;">
        <h2 style="color: #8b5cf6; margin-bottom: 16px;">Drago nam je što si tu</h2>
        <p style="color: #94a3b8; line-height: 1.6;">
        ...
        <a href="${dashboardUrl}" style="display: inline-block; margin: 24px 0; padding: 12px 32px; background: #8b5cf6; color: white; text-decoration: none; border-radius: 12px; font-weight: 600;">
          Počni razgovor
        </a>
        <p style="color: #94a3b8; font-size: 14px; line-height: 1.6;">
...
export async function sendGoodbyeEmail(to: string) {
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #0a0a12; color: #e2e8f0; border-radius: 16px;">
        <h2 style="color: #8b5cf6; margin-bottom: 16px;">Žao nam je što odlaziš</h2>
        <p style="color: #94a3b8; line-height: 1.6;">
